...
> **Note for reviewers — read this first.** Everything below this section is
> the original assignment brief, kept exactly as given. This part is my own
> summary of the finished submission.

## MediaVault — final submission

A hardened rebuild of the MediaVault asset library given below: correct
under the API's chaos/latency, fast at 12,400+ rows, fully usable from a
keyboard, and restrained rather than decorated.
...
Starts the mock API (`:8787`) and the app (`:5173`) together, chaos and
latency on by default — no extra config needed.

### What's implemented

- **Search & filtering** — debounced (350ms), synced to the URL, immune to
  stale responses: every distinct filter combination gets its own cache
  slot, and an obsolete in-flight request is genuinely aborted, not just
  ignored (verified live in the network log).
- **Scale** — cursor-based infinite scroll with row virtualization
  (`@tanstack/react-virtual`); rendered DOM stays flat (~40 nodes) no matter
  how far you've scrolled through the 12,400 assets.
- **Bulk actions** — range selection, optimistic updates, chunked requests
  with bounded concurrency, and real handling of partial success (`207`):
  only the ids that actually failed roll back, with a retry for the ones
  worth retrying.
- **Resilience** — exponential backoff with jitter, honors `Retry-After`,
  retry/no-retry decided structurally (never by string-matching an error
  message), offline detection, and an error boundary.
- **Accessibility** — the whole grid is keyboard-operable via a roving
  tabindex (arrows, Enter, Space, Shift+Arrow, Escape), live-region
  announcements, and real ARIA grid semantics.
- **Interface** — one small design-token system, status shown as a
  progression rather than color alone, and contrast checked against the
  actual WCAG formula, not eyeballed.

### Design decisions and trade-offs

The full account — every decision and what was rejected, measured
performance numbers, accessibility testing notes (including the honest
gaps, like no screen-reader pass), and what was deliberately left out — is
in [`SUBMISSION.md`](./SUBMISSION.md). Worth reading before the code itself.

### Notes for the reviewer

- `server/` and `API.md` are untouched, as required.
- Commit history is real and unsquashed.
- Screenshots of the final UI are in `docs/screenshots/`.
